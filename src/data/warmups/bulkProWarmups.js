export const bulkProWarmups = [
  {
    id: "warmup-d1",
    title: "Warm-up — D1 Chest & Triceps",
    goal: "Prepare pressing pattern, shoulders, and elbows",
    steps: [
      {
        title: "Light cardio",
        items: ["5 min easy bike / walk / stepper"],
      },
      {
        title: "Shoulder + pressing prep",
        items: [
          "Band pull-apart 12-15",
          "Face pull or cable external rotation 12-15",
          "Push-up or empty-bar press 8-10",
        ],
      },
      {
        title: "Ramp sets for bench",
        items: ["8 @ ~40%", "5 @ ~60%", "3 @ ~75%", "No fatigue"],
      },
    ],
  },
  {
    id: "warmup-d2",
    title: "Warm-up — D2 Back & Biceps",
    goal: "Prepare scapular control, lats, and elbows",
    steps: [
      {
        title: "Light cardio",
        items: ["5 min easy bike / walk / stepper"],
      },
      {
        title: "Scap + lat activation",
        items: [
          "Scapular pull-up or dead hang 6-8",
          "Straight-arm pulldown 10-12",
          "Light pulldown 8-10",
        ],
      },
      {
        title: "Ramp sets for pull-up",
        items: [
          "Easy bodyweight or assisted set",
          "1-2 controlled ramp sets",
          "Stop before grip fatigue",
        ],
      },
    ],
  },
  {
    id: "warmup-d3",
    title: "Warm-up — D3 Quads Heavy",
    goal: "Prepare squat pattern and lower body",
    steps: [
      {
        title: "Light cardio",
        items: ["5 min easy bike / walk / stepper"],
      },
      {
        title: "Mobility + activation",
        items: [
          "Hip hinge drill 8-10",
          "Bodyweight squat 8-10",
          "Glute bridge 10-12",
          "Ankle rocks 10/10",
        ],
      },
      {
        title: "Ramp sets for squat",
        items: ["8 @ ~40%", "5 @ ~60%", "3 @ ~75%", "No fatigue"],
      },
    ],
  },
  {
    id: "warmup-d4",
    title: "Warm-up — D4 Shoulders & Arms Light",
    goal: "Prepare shoulders, upper back, and elbows",
    steps: [
      {
        title: "Light cardio",
        items: ["5 min easy bike / walk / stepper"],
      },
      {
        title: "Shoulder prep",
        items: [
          "Band pull-apart 12-15",
          "Cable or band external rotation 10-12 / side",
          "Light lateral raise 10-12",
        ],
      },
      {
        title: "Ramp sets for overhead press",
        items: [
          "8 with empty bar or light load",
          "5 @ moderate load",
          "3 clean reps",
          "No grind",
        ],
      },
    ],
  },
  {
    id: "warmup-d5",
    title: "Warm-up — D5 Chest Pump & Rows",
    goal: "Prepare chest, lats, and upper back without wasting energy",
    steps: [
      {
        title: "Light cardio",
        items: ["5 min easy bike / walk / stepper"],
      },
      {
        title: "Upper-body activation",
        items: ["Band pull-apart 12-15", "Push-up 8-10", "Light row 10-12"],
      },
      {
        title: "Ramp sets for first press / row",
        items: [
          "1-2 light ramp sets",
          "Keep reps smooth",
          "No fatigue before working sets",
        ],
      },
    ],
  },
  {
    id: "warmup-d6",
    title: "Warm-up — D6 Posterior Chain",
    goal: "Prepare hip hinge, hamstrings, and lower back",
    steps: [
      {
        title: "Light cardio",
        items: ["5 min easy bike / walk / stepper"],
      },
      {
        title: "Hinge + glute activation",
        items: [
          "Hip hinge drill 8-10",
          "Glute bridge 10-12",
          "Bodyweight RDL 8-10",
        ],
      },
      {
        title: "Ramp sets for RDL",
        items: [
          "8 @ light load",
          "5 @ moderate load",
          "3 @ working approach load",
          "No lower-back fatigue",
        ],
      },
    ],
  },
];
